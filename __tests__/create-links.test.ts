...
describe("createLinks", () => {
	...
	test("should return empty object if range is null", () => {
		expect(createLinks({ limit, offset, range: null, type, method })).toEqual(
			{}
		);
	});
	...
	describe("when range is not null", () => {
		...
		test("should return prev and next links when appropriate", () => {
			...
			const links = createLinks({ limit, offset, range, type, params, method });
		...
		test("should return prev and clamp at 0", () => {
			...
			expect(
				createLinks({ limit: 90, offset: 80, range, type, method })
			).toEqual(expected);
		});
		...
		test("should return only next", () => {
			...
			expect(
				createLinks({ limit: 60, offset: 0, range, type, method })
			).toEqual(expected);
		});
		...
		test("should return only prev link when at end of range", () => {
			...
			expect(createLinks({ limit, offset: 90, range, type, method })).toEqual(
				expected
			);
		});
		...
		test("should return only next link when at start of range", () => {
			...
			expect(createLinks({ limit, offset: 0, range, type, method })).toEqual(
				expected
			);
		});
		...
		test("should handle undefined URLSearchParams", () => {
			...
			expect(createLinks({ limit, offset, range, type, method })).toEqual(
				expected
			);
		});
	});
