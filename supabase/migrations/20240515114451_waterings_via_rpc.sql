...
CREATE OR REPLACE FUNCTION public.waterings_for_tree(t_id text)
 RETURNS TABLE(uuid text, amount numeric, "timestamp" timestamptz, username text, id int4, tree_id text)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
BEGIN
	...
	SELECT * FROM trees_watered WHERE trees_watered.tree_id = t_id;
...
BEGIN
	...
	SELECT * FROM trees_watered WHERE trees_watered.uuid = u_id;
